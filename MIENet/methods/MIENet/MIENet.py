...
class IRM_Atrous(nn.Module):
    def __init__(self, in_channels, out_channels):
        super(IRM_Atrous, self).__init__()
...
class IRM_front(nn.Module):
    def __init__(self, in_channels, num_masks):
        super(IRM_front, self).__init__()
...
class MIENet(BasicModelClass):
    def __init__(self, last_module=ASPP):
        ...
        self.merge_layers = nn.ModuleList([FSMM(in_dim=in_c) for in_c in (2048, 1024, 512, 256, 64)])
        ...
        self.iter_layer_1 = IRM_front(32, 1)
        self.iter_layer_2 = IRM_Atrous(32, 32)
    ...
    def train_forward(self, data, **kwargs):
        assert not {"image4.0", "image1.0", "image0.25", "mask"}.difference(set(data)), set(data)

        output = self.body(
            l_scale=data["image4.0"],
            m_scale=data["image1.0"],
            s_scale=data["image0.25"],
        )
